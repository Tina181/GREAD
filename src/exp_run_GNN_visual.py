...
def train(model, optimizer, data, pos_encoding=None):
    ...
    loss = lf(out[data.train_mask], data.y.squeeze()[data.train_mask])  # data.train_mask.shape: torch.Size([3327]), data.y.shape: torch.Size([2120])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_best_params_aggdiff', action='store_true', help="flag to take the best params of Aggeragation Diffusion")
    ...
    # gread args
    parser.add_argument('--reaction_term', type=str, default='bspm', help='bspm, fisher, allen-cahn, aggdiff-gat, aggdiff-log, aggdiff-sin')
    parser.add_argument('--beta_diag', type=eval, default=False)
    parser.add_argument('--log_eps', type=float, default=1.0, help='log(eps+ |u-v|), balancing attraction and repulsion')
    parser.add_argument('--diffusion_rate1', type=float, default=1.0, help='diffusion rate for diffusion term')
    parser.add_argument('--diffusion_rate2', type=float, default=1.0, help='diffusion rate for reaction term')
    parser.add_argument('--time_encoding', type=str, default="None", help='None, mlp: using time dependent params')
